...
from notehub.commands import show, list as list_cmd, status, add, edit
...
class TestShowRun:
    ...
    def test_show_multiple_issues(self, mocker, capsys):
        ...
        mocker.patch('notehub.commands.show.resolve_note_ident', 
                    side_effect=[(42, None), (43, None)])
        
        mock_issues = [
            {'number': 42, 'title': 'First', 'html_url': 'https://github.com/testorg/testrepo/issues/42'},
            {'number': 43, 'title': 'Second', 'html_url': 'https://github.com/testorg/testrepo/issues/43'}
        ]
        mocker.patch('notehub.commands.show.get_issue', side_effect=mock_issues)
        mocker.patch('notehub.commands.show.format_note_header', 
...
class TestListRun:
    ...
    def test_list_success(self, mocker, capsys):
        ...
        mocker.patch('notehub.commands.list.format_note_header', 
                    side_effect=['#42: First', '#43: Second'])
...
class TestCLI:
    ...
    def test_common_args_on_all_commands(self):
        ...
        args = parser.parse_args(['add', '--host', 'gh.example.com', '--org', 'myorg', '--repo', 'myrepo', '--global'])
